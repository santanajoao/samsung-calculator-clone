...
const addOperationsListener = () => {
  ...
  allOperators.forEach(
    (element) => element.addEventListener('click', displayOperation),
  );
};
...
const isNumber = (caractere) => !Number.isNaN(caractere);
...
const isSafeExpression = (expression) => {
  /* A função eval executa código JavaScript podendo ser insegura. Essa função
  testa se a expressão contem apenas os caracteres matemáticos esperados */
